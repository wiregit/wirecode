...
public class CountPercent {
    ...
    private static final String ENGLISH_LINK ="http://www.limewire.org/fisheye/viewrep/~raw,r=MAIN/limecvs/lib/messagebundles/MessagesBundle.properties";
	...
	private void loadFile(Map langs, InputStream in, Set advanced, String filename) {
        ...
        try {
            ...
            langs.put(new LanguageInfo(lc, cc, vc, ln, cn, vn, sn, dn, filename), p);
        } catch(IOException e) {
            // ignore.
        } finally {
            if( in != null )
    ...
    private void buildStartOfPage(StringBuffer page) {
        page.append(
            ...
            "translations.<br>\n" +
            "<br>\n" + 
            "<a href=\"http://limewire.org/fisheye/viewrep/~raw,r=MAIN/limecvs/lib/messagebundles/MessagesBundle_es.properties\"><b>Click " +
    ...
    private void buildStatus(StringBuffer page, List completed, List midway, List started) {
        ...
        page.append(" are complete and will require only small revisions during the proejct evolution.</li>");
...
"//--></script><noscript><a href=\"#\">(Email address protected by JavaScript: \n " +
  ...
  "</td>\n" + 
    ...
    "<tr bgcolor=\"#D9E3F2\">\n" + 
      ...
      "href=\"http://www.limewire.org/fisheye/viewrep/~br=MAIN/limecvs/lib/messagebundles\">click here</a></font>.\n"
    ); 
    ...
    private void buildProgress(StringBuffer page, Map charsets) {
        ...
        for(Iterator i = latin.iterator(); i.hasNext(); ) {
            LanguageInfo l = (LanguageInfo)i.next();
            page.append("<tr>\n<td>");
            page.append("<b>" + l.getLink() + "</b></td>\n");
            page.append("<td align=\"center\">(" + pc.format(l.getPercentage()) + "%)</td>\n");
            page.append("<td align=\"center\">" + l.getCode() + "</td>\n");
            page.append("</tr>");
        }
        ...
        for(Iterator i = charsets.entrySet().iterator(); i.hasNext(); ) {
            ...
            for(Iterator j = l.iterator(); j.hasNext(); ) {
                ...
                page.append("<tr>\n<td>");
                page.append("<b>" + li.getLink() + "</b></td>\n");
                page.append("<td align=\"center\">(" + pc.format(li.getPercentage()) + "%)</td>\n");
                page.append("<td align=\"center\">" + li.getCode() + "</td>\n");
                page.append("</tr>");
            }
        }
...
class LanguageInfo implements Comparable {
    private static final String PRE_LINK = "http://www.limewire.org/fisheye/viewrep/~raw,r=MAIN/limecvs/lib/messagebundles/";    
    ...
    public String getLink() {
        return "<a href=\"" + PRE_LINK + fileName + "\">" + displayName + "</a>";
    }
