...
@Singleton
public class SearchResultPropertiesImpl extends Dialog implements SearchResultProperties {
    ...
    public void showProperties(VisualSearchResult vsr) {
        ...
        for(SearchResult result : vsr.getCoreSearchResults()) {
            for(RemoteHost host : result.getSources()) {
                readOnlyInfoModel.addRow(new Object[] { host.getRenderName(), result.getFileName()});
            }
        }
