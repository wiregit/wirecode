...
public class RemoteHostActionsImpl implements RemoteHostActions {
    ...
    public void viewLibraryOf(RemoteHost person) {
        ...
        Search search = new BrowseSearch(remoteLibraryManager, browseFactory, person.getFriendPresence());
        ...
        SearchNavItem item = searchNavigator.get().addSearch(
                I18n.tr("Browse: {0}", person.getFriendPresence().getFriend().getRenderName()), searchPanel, search);
        item.select();
