 ...
 */
@Singleton
public class TicTacToeFriendListPane extends JPanel {
    ...
    private final EventList<ChatFriend> ticktactoeFriends;
    ...
    @Inject
    public TicTacToeFriendListPane(IconLibrary icons, LibraryNavigator libraryNavigator) {
        super(new BorderLayout());
        this.icons = icons;
        this.ticktactoeFriends = new BasicEventList<ChatFriend>();
        ...
        ObservableElementList<ChatFriend> observableList = GlazedListsFactory.observableElementList((EventList<ChatFriend>)ticktactoeFriends, GlazedLists.beanConnector(ChatFriend.class));
        SortedList<ChatFriend> sortedFriends = GlazedListsFactory.sortedList(observableList,  new FriendAvailabilityComparator());
//        ObservableElementList<TicTacToeFriend> observableList = GlazedListsFactory.observableElementList(ticktactoeFriends, GlazedLists.beanConnector(TicTacToeFriend.class));
//        SortedList<TicTacToeFriend> sortedFriends = GlazedListsFactory.sortedList(observableList,  
//                new FriendAvailabilityComparator());
        ...
        public boolean isHighlighted(Component renderer, ComponentAdapter adapter) {
            ...
            if (chatFriend.isChatting()) {
                ...
                if (model.getRowCount() > (row + 1)) {
                    TicTacToeFriend nextFriend = (TicTacToeFriend) model.getElementAt(row + 1);
    ...
    private void handlePresenceEvent(FriendPresenceEvent event) {
        ...
        ChatFriend tictacktoeFriend = idToFriendMap.get(user.getId());
        switch(event.getType()) {
        case ADDED:
            if(tictacktoeFriend == null) {
                System.out.println("cast in TicTacToeFriendListPane");
                tictacktoeFriend = new ChatFriendImpl(presence);
                ticktactoeFriends.add((ChatFriend)tictacktoeFriend);
                idToFriendMap.put(user.getId(), tictacktoeFriend);
            }

            System.out.println("3 casts in tictactoefriendlistpane");
            final ChatFriend tictacktoeFriendForIncomingMove = tictacktoeFriend;
            IncomingChatListener incomingChatListener = new IncomingChatListener() {
                public MessageReader incomingChat(MessageWriter writer) {
                    ...
                    MessageWriter writerWrapper = new MessageWriterImpl(myID,(ChatFriend)tictacktoeFriendForIncomingMove, writer);
                    ConversationSelectedEvent event =
                            new ConversationSelectedEvent((ChatFriend)tictacktoeFriendForIncomingMove, writerWrapper, false);
                    event.publish();
                    //Hang out until a responder has processed this event
                    event.await();
                    return new MessageReaderImpl((ChatFriend)tictacktoeFriendForIncomingMove);
                }
            };
            user.setChatListenerIfNecessary(incomingChatListener);
            tictacktoeFriend.update();
            break;
        case UPDATE:
            if (tictacktoeFriend != null) {
                tictacktoeFriend.update();
        ...
        case REMOVED:
            if (tictacktoeFriend != null) {
                if (shouldRemoveFromFriendsList(tictacktoeFriend)) {
                    ticktactoeFriends.remove(idToFriendMap.remove(user.getId()));
                }
                tictacktoeFriend.update();
            }
            break;
    ...
    private void startOrSelectConversation(ChatFriend tictacktoeFriend) {
        System.out.println("not in TicTacToeFriendListPane");
//        MessageWriter writerWithEventDispatch = null;
//        if (!tictacktoeFriend.isChatting() && tictacktoeFriend.isSignedIn()) {
//            MessageWriter writer = tictacktoeFriend.createChat(new MessageReaderImpl(tictacktoeFriend));
//            writerWithEventDispatch = new MessageWriterImpl(myID, tictacktoeFriend, writer);
//        }
//        new ConversationSelectedEvent(tictacktoeFriend, writerWithEventDispatch, true).publish();
//        setActiveConversation(tictacktoeFriend);
        System.out.println("not setting active conversation in TicTacToeFriendListPane");
    }

    private void setActiveConversation(ChatFriend chatFriend) {
//        LOG.debugf("Setting active conversation: {0}", chatFriend.getName());
//        TicTacToeFriend oldActiveConversation = activeConversation.get();
//        if (oldActiveConversation != null) {
//            oldActiveConversation.setActiveConversation(false);
//        }
//        activeConversation = new WeakReference<TicTacToeFriend>(chatFriend);
...
//        friendTimerMap.remove(chatFriend);
    ...
    private Icon getChatIcon(ChatFriend chatFriend) {
        ...
        System.out.println("need to fix tictactoefriendlistpane");
        return getIcon((ChatFriend )chatFriend, icons);
    }
    ...
    public void handleCloseChatEvent(CloseChatEvent event) {
        closeTicTackToe(event.getFriend());
    }

    private void closeTicTackToe(ChatFriend chatFriend) {
        if (chatFriend != null) {

            chatFriend.stopChat();
            if (!chatFriend.isSignedIn()) {
                ticktactoeFriends.remove(idToFriendMap.remove(chatFriend.getID()));
            }
            System.out.println("another cast, chatfriend");
            ChatFriend nextFriend = null;
            for(ChatFriend tmpFriend : ticktactoeFriends) {
    ...
    private void closeAllChats() {
        for (ChatFriend chatFriend : ticktactoeFriends) {
    ...
    private class CustomTooltipLocationTable extends JXTable {
        ...
        public String getToolTipText(MouseEvent event) {
            ...
            TicTacToeFriend chatFriend = (TicTacToeFriend) model.getElementAt(row);
        ...
        public void start() {
            Timer timer = new Timer(1500, new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    int friendIndex = ticktactoeFriends.indexOf(chatFriend);
                    if (friendIndex > -1) {
        ...
        public void mouseMoved(MouseEvent e) {
            ...
            if (friend == null) {
                ...
                if (previousMouseHoverFriend != null) {
                    ...
                    int previousMouseHoverFriendIndex = ticktactoeFriends.indexOf(previousMouseHoverFriend);
            ...
            int friendIndex = ticktactoeFriends.indexOf(friend);
            if (friendIndex > -1) {
