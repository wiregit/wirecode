...
import com.google.inject.Singleton;
    ...
    @Singleton
    public class TicTacToePanel extends JPanel implements Displayable {
        private static final Log LOG = LogFactory.getLog(TicTacToePanel.class);
        private final TicTacToePaneFactory tictactoeFactory;
        private final JPanel conversationPanel;
        private final TicTacToeFriendListPane friendsPanel;
        private final Map<String, ConversationPane> chats;
        private final ChatTopPanel chatTopPanel;
        
        @Inject
        public TicTacToePanel(TicTacToePaneFactory tictactoeFactory, IconLibrary icons, TicTacToeFriendListPane friendsPanel,
        ...
        @EventSubscriber
        public void handleSelectedConversation(TicTacToeSelectedEvent event) {
            TicTacToeFriend tictactoeFriend = event.getFriend();
            LOG.debugf("ConversationSelectedEvent with friend: {0}", tictactoeFriend.getName());
            ConversationPane chatPane = chats.get(tictactoeFriend.getID());
            if (chatPane == null) {
                System.out.println("TicTacToePanel!!!!!");
//                chatPane = tictactoeFactory.create(event.getWriter(), tictactoeFriend, getLoggedInID());
                chats.put(tictactoeFriend.getID(), chatPane);
            }
