...
public final class QueryKey implements SecurityToken {
    ...
    /**
     * The QueryKey settings provider
     */
    ...
    /**
     * The time when a new secretKey was created
     */
    private static long lastQueryKeyChange = 0L;
    ...
    public static QueryKey getQueryKey(InetAddress ip, int port) {
        return new QueryKey(getSecretKey().getKeyBytes(ip,port), true);
    }
     ...
     * Creates a new Key
     */
    private static void newKeyGenerator() {
        synchronized (LOCK) {
            oldSecretKey = secretKey;
            secretKey = createKeyGenerator();
            lastQueryKeyChange = System.currentTimeMillis();
