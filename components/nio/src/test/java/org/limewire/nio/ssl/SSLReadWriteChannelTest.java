...
public class SSLReadWriteChannelTest extends BaseTestCase {
    ...
    // NOTE -- Many of the tests offload initializing, writing & reading to the
    // NIODispatcher
    // Thread. This is because after the SSLReadWriteChannel finishes a task, if
    // there
    // was buffered read data it will trigger a read in that thread. If there
    // are other
    // reads occurring in other threads, that can cause internal corruption in
    // the
    // SSLEngine. So, all reading/writing/initializing must be done in a single
    // thread.
    ...
    public void testClientTLSFails() throws Exception {
        ...
        SSLReadWriteChannel channel = new SSLReadWriteChannel(context, executor, NIODispatcher
                .instance().getBufferCache(), NIODispatcher.instance()
                .getScheduledExecutorService());
        channel.initialize(null, new String[] { "TLS_DH_anon_WITH_AES_128_CBC_SHA" }, true, false);

        ReadBufferChannel readSink = new ReadBufferChannel(StringUtils
                .toAsciiBytes("NOT TLS DATA!"));
        WriteBufferChannel writeSink = new WriteBufferChannel(300);
        channel.setReadChannel(readSink);
        channel.setWriteChannel(writeSink);
        new WriteBufferChannel(StringUtils.toAsciiBytes("OUTGOING"), channel);
    ...
    public void testNonBlockingTLSExchange() throws Exception {
        ...
        new WriteBufferChannel(StringUtils.toAsciiBytes(serverOut), serverChannel);
        new WriteBufferChannel(StringUtils.toAsciiBytes(clientOut), clientChannel);
        ...
        assertEquals(serverOut, StringUtils.getASCIIString(clientRead.array(), 0, clientRead.position()));
        assertEquals(clientOut, StringUtils.getASCIIString(serverRead.array(), 0, serverRead.position()));
    ...
    public void testShutdown() throws Exception {
        ...
        final SSLReadWriteChannel clientChannel = new SSLReadWriteChannel(context, executor,
                NIODispatcher.instance().getBufferCache(), NIODispatcher.instance()
                        .getScheduledExecutorService());
        ...
        NIODispatcher.instance().getScheduledExecutorService().submit(new Runnable() {
            public void run() {
                clientChannel.initialize(null, new String[] { "TLS_DH_anon_WITH_AES_128_CBC_SHA" },
                        true, false);
                serverChannel.initialize(null, new String[] { "TLS_DH_anon_WITH_AES_128_CBC_SHA" },
                        false, false);
            }
        }).get();
        ...
        for (int i = 0; i < 10; i++) {
            NIODispatcher.instance().getScheduledExecutorService().submit(new Runnable() {
                public void run() {
                    try {
                        clientChannel.shutdown(); // these do nothing, since the
                                                  // Pipes are still open
                        serverChannel.shutdown();
                        clientChannel.read(clientRead);
                        serverChannel.read(serverRead);
                        clientChannel.handleWrite();
    ...
    public void testShutdownCancelsInitialize() throws Exception {
        ...
        cache.clearCache();
        // Test shutting down before initialize -- no session or buffers
        // created.
        channel1.shutdown();
        SSLSession session1 = NIODispatcher.instance().getScheduledExecutorService().submit(
                new Callable<SSLSession>() {
                    public SSLSession call() {
                        channel1.initialize(null,
                                new String[] { "TLS_DH_anon_WITH_AES_128_CBC_SHA" }, true, false);
        ...
        cache.clearCache();
        // And initializing before shutting down will create the buffers /
        // session,
        // but later shutting will will then remove them.
        SSLSession session2 = NIODispatcher.instance().getScheduledExecutorService().submit(
                new Callable<SSLSession>() {
                    public SSLSession call() {
                        channel2.initialize(null,
                                new String[] { "TLS_DH_anon_WITH_AES_128_CBC_SHA" }, true, false);
        ...
        assertEquals(session2.getPacketBufferSize() + session2.getPacketBufferSize(), cache
                .getHeapCacheSize());
    }
    ...
    public void testParentInterestOffDoesntKillHandshake() throws Exception {
        ...
        String serverOut = "I AM A SERVER\r\n, HELLO\r\n";
        String clientOut = "A CLIENT I AM\r\n, GOODBYE\r\n";
        final InterestWritableByteChannel serverSource = new WriteBufferChannel(StringUtils
                .toAsciiBytes(serverOut), serverChannel);
        final InterestWritableByteChannel clientSource = new WriteBufferChannel(StringUtils
                .toAsciiBytes(clientOut), clientChannel);
        ...
        assertEquals(serverOut, StringUtils.getASCIIString(clientRead.array(), 0, clientRead
                .position()));
        assertEquals(clientOut, StringUtils.getASCIIString(serverRead.array(), 0, serverRead
                .position()));
    ...
    public void testHasBufferedOutput() throws Exception {
        ...
        ByteBuffer buffer = ByteBuffer.wrap(StringUtils.toAsciiBytes("Hello"));
    ...
    public void testEOFDuringUnderflow() throws Exception {
        ...
        final SSLReadWriteChannel clientChannel = new SSLReadWriteChannel(context, executor,
                NIODispatcher.instance().getBufferCache(), executor);
        final SSLReadWriteChannel serverChannel = new SSLReadWriteChannel(context, executor,
                NIODispatcher.instance().getBufferCache(), executor);
    ...
    private void doTLSTest(final boolean testServer) throws Exception {
        ...
        NIODispatcher.instance().getScheduledExecutorService().submit(new Runnable() {
            public void run() {
                channel.initialize(accepted.getRemoteSocketAddress(),
                        new String[] { "TLS_DH_anon_WITH_AES_128_CBC_SHA" }, testServer, false);
            }
    ...
    private void doDataTest(final SSLReadWriteChannel channel, final SSLSocket sslSocket,
        ...
        Thread t = new ManagedThread(new Runnable() {
            public void run() {
                try {
                    ...
                    sslSocket.getOutputStream().write(StringUtils.toAsciiBytes(INCOMING));
                } catch (IOException iox) {
                    throw new RuntimeException(iox);
                }
            }
        ...
        assertEquals(INCOMING, StringUtils.getASCIIString(channelRead.array(), 0, channelRead.position()));
        assertEquals(OUTGOING, StringUtils.getASCIIString(b, 0, OUTGOING.length()));
