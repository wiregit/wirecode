...
public final class AutoUpdateHelperImpl implements AutoUpdateHelper{
    ...
    public boolean isUpdateAvailable() {
        ...
        try{
            ...
            }else if( LimeWireUtils.isBetaRelease() && updateVersion.compareTo(limewireVersion) == 0){
                updateAvailable = true;
            }
        }catch(VersionFormatException ex){
            LOG.warn("Error parsing version info. ");
            LOG.warn("Current Version: " + LimeWireUtils.getLimeWireVersion());
            LOG.warn("Update Version: " + UpdateSettings.AUTO_UPDATE_VERSION.get());
        }
    ...
    public boolean isUpdateReadyForInstall(){
        return (isUpdateAvailable() && UpdateSettings.AUTO_UPDATE_COMMAND.get() != "");
    }
    
    @Override
    public File getTemporaryWorkingDirectory(){
        File tempDirectory = new File(CommonUtils.getUserSettingsDir(), "autoupdate");
    ...
    public boolean downloadUpdates(){
        ...
        try{ 
            ...
            downloadCommand = downloadCommand.replace("${system_temp_directory}", temporaryDirectory.getAbsolutePath());
            File downloadScript = createExecutableScriptFile(temporaryDirectory, "download", downloadCommand);
            ...
            if(process.waitFor() == 0){
                ...
                File installScript = createExecutableScriptFile(temporaryDirectory, "install", installCommand);
    ...
    public File getAutoUpdateCommandScript(){
        String downloadCommand = getDownloadCommand();
        File downloadScript = createExecutableScriptFile(getTemporaryWorkingDirectory(), "download", downloadCommand);
    ...
    private String getInstallCommands(String installerPath){
        ...
        String cmd = null;
        if(OSUtils.isLinux()){
            cmd = "gdebi-gtk " + installerPath + "\n limewire";
        }else if(OSUtils.isMacOSX()){
            cmd = "hdiutil attach " + installerPath + "\n limewire.app";
