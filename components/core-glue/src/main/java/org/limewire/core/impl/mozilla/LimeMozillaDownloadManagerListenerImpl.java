...
public class LimeMozillaDownloadManagerListenerImpl implements
    ...
    private synchronized void resumeDownloads() {
        ...
        while (enumerator.hasMoreElements()) {
            ...
            } catch (XPCOMException ignored) {
                //can still pause/resume from the ui
            }
        }
    }
    ...
    public void removeListener(
        ...
        MozillaExecutor.mozSyncExec(new Runnable() {
            @Override
            public void run() {
                synchronized (LimeMozillaDownloadManagerListenerImpl.this) {
                    if (listeners.containsKey(limeMozillaDownloadProgressListener.getDownloadId())) {
                        nsIDownloadManager downloadManager = getDownloadManager();
                        try {
                        ...
                        listeners.remove(limeMozillaDownloadProgressListener.getDownloadId());
                    }
                }
            }
        });
