...
public abstract class BaseTestCase extends AssertComparisons implements UncaughtExceptionHandler,
    ...
    /**
     * Build a test suite containing a single test from a specified test class
     * 
     * @param cls The test class (must be subclassed from TestCase)
    ...
    /**
     * Build a test suite containing a set of tests from a specified test class
     * 
     * @param cls The test class (must be subclassed from TestCase)
    ...
    /**
     * Used in conjunction with <code>BaseTestCase</code>'s implementation of
     * <code>ErrorService</code>. Allows a test to configure an expected
     * <code>Exception</code>.
     * 
    ...
    /*
     * This is modified to run 'preSetUp' and 'postTearDown' as methods which
     * all tests will run, regardless of their implementation (or lack of) of
     * setUp and tearDown.
     * 
     * It is also modified so that if setUp throws something, tearDown will
     * still be run.
     */
    @Override
    public void runBare() throws Throwable {
        _currentTestName = getName();
        if (System.getProperty("junit.test.hidetestname", "${hidetestname}").equals(
                "${hidetestname}")) {
            System.out.println("Running test: " + _currentTestName);
            // LogFactory.getLog(this.getClass()).info("Running test: " +
            // _currentTestName);
        }
    ...
    /**
     * Intercepted to allow us to get a handle to the test result, so we can add
     * errors from the ErrorService callback (giving us errors that were
    ...
    /**
     * Called before each test's setUp. Used to determine which thread the test
     * is running in, set up the testing directories, and possibly print
     * debugging information (such as the current test being run) This must also
     * set the ErrorService's callback, so it associates with the correct test
     * object.
     */
    ...
    /**
     * Called after each test's tearDown. Used to remove directories and
    ...
    /**
     * Sets up the TimerTask to kill the running test after a certain amount of
     * time.
     */
    private final void setupTestTimer() {
        ...
        _testKiller = new TimerTask() {
            ...
            public void run() {
                ...
                error(new RuntimeException("Stalled!  Took " + (now - _startTimeForTest) + " ms."),
                        "Test Took Too Long");
            }
        };
        // kill in a bit.
    ...
    /**
     * Fails the test with an AssertionFailedError and another error as the root
     * cause.
     */
     ...
     * ErrorCallback.
     * <p>
     * It is used to catch errors that may or may not be inside of the test
     * thread. If it is in the thread, we can just rethrow the error, and the
     * test will fail as normal. If it is outside of the thread, we want the
     * test results to remember the error, but we must allow the test to
     * continue as normal, possibly succeeding, failing or erroring.
     * <p>
     * If an <code>expectedException</code> has been set, then this method will
     * check to see if the incoming exception is of the correct class. If it is,
     * then the call to this method is essentially ignored.
     * <p>
     * Note that while the XML formatter can easily handle the case of multiple
     * failures/errors in a single test, the XML->HTML converter doesn't do that
     * good of a job. It correctly lists the amount of errors/failures, but it
     * will only write the last one as the status of the test, and will also
     * only write the last one as the message/stacktrace.
     */
    public void error(Throwable ex, String detail) {
        ...
        ex = new UnexpectedExceptionError(detail, ex); // remember the detail &
                                                       // stack trace of the
                                                       // ErrorService.
        if (_testThread != Thread.currentThread()) {
            // the Eclipse JUnit plug-in does not report multiple errors per
            // test case:
            // https://bugs.eclipse.org/bugs/show_bug.cgi?id=125296
            // print out stack trace to make debugging of uncaught exceptions
            // easier
    ...
    /**
     * Returns a test which will fail and log a warning message. Copied from
     * JUnit's TestSuite.java Note that it does not have to extend BaseTestCase,
     * just TestCase. BaseTestCase would add needless complexity to an otherwise
