...
public class GUID implements Comparable<GUID> {
    ...
    /** 
     * generate a new GUID using md5 of a public Key and sturgeonID
     */
    public static byte[] makeSecureGuid(byte[] multiInstallationMark, byte[] publicKeyMd5Value, boolean tagging) {
        assert(multiInstallationMark.length == 3);
        assert(publicKeyMd5Value.length == 16);
        byte[] ret = publicKeyMd5Value;
        if(tagging){
            //Apply common tags.
            ret[15]=(byte)0x00;   //Version number is 0.
            //Apply LimeWire's marking.
        ...
        ret[12]=multiInstallationMark[0];
        ret[13]=multiInstallationMark[1];
        ret[14]=multiInstallationMark[2];
        return ret;
    }

    /** 
     * verify is the secureID is generated by the public key 
     */
    public static boolean isSecureGuid(byte[] publicKeyMd5Value, GUID secureId, boolean tagging) {
        if (publicKeyMd5Value.length != 16){
            return false;
        }
        byte[] ret = publicKeyMd5Value;
        if(tagging){
            if (secureId.bytes()[15] != (byte)0x00)
                return false;
            //Apply LimeWire's marking.
            tagGuid(ret, 4, 6, 9);
        }
        byte[] secureIdBytes = secureId.bytes();
        // 12, 13 and 14 are from multiInstallationMark
        for(int i=0; i<12; i++){ 
            if(ret[i]!=secureIdBytes[i]){
                return false;
            }
        }
        return true;
