...
public class GUID implements Comparable<GUID> {
     ...
     */
    public static byte[] makeSecureGuid(GUID sturgeonID, byte[] publicKeyMd5Value) {
        ...
        //Apply LimeWire's marking.
        tagGuid(ret, 4, 6, 9);
        //combine with sturgeonID, taking two bytes from sturgeonID
        byte[] sturgeonBytes = sturgeonID.bytes();        
        ret[13]=sturgeonBytes[13];
        ret[14]=sturgeonBytes[14];
        return ret;
    }
    ...
    public static boolean isSecureGuid(byte[] publicKeyMd5Value, GUID secureId) {
        if (publicKeyMd5Value.length != 16 || secureId.bytes()[15]!=(byte)0x00){
            return false;
        }
        byte[] ret = publicKeyMd5Value;
        //Apply LimeWire's marking.
        ...
        // 13 and 14 are from sturgeonID, 15 is 00
        for(int i=0; i<13; i++){ 
