...
public abstract class RemoteServer extends AbstractServer {
    ...
    public RemoteServer(final int port, final String name, final DispatcherImpl dis) {
        super(port, name, dis);
    }

    public RemoteServer(final int port, final DispatcherImpl dis) {
    ...
    public abstract class DispatcherImpl extends DispatcherSupport {
        ...
        class StoreKey extends AbstractHandler {
            ...
            public String handle(final Map<String, String> args) {
                ...
                String publicKey = getArg(args, DispatcherSupport.Parameters.PUBLIC);
                if (publicKey == null) {
                    return report(DispatcherSupport.ErrorCodes.MISSING_PUBLIC_KEY_PARAMETER);
                }
                String privateKey = getArg(args, DispatcherSupport.Parameters.PRIVATE);
                if (privateKey == null) {
                    return report(DispatcherSupport.ErrorCodes.MISSING_PRIVATE_KEY_PARAMETER);
                }
                String ip = getArg(args, DispatcherSupport.Parameters.IP);
                if (ip == null) {
...
//                    ip = "localhost:8080";
     ...
     */
    protected abstract boolean storeKey(String publicKey, String privateKey,
            String ip);
