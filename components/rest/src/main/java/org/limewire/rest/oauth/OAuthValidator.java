...
/**
 * An agent used to validate requests using the OAuth protocol.  At present,
...
public class OAuthValidator {
     ...
     * and consumer secret.
     */
    public OAuthValidator(String baseUrl, int port, String secret) {
    ...
    private String createBaseUrl(String baseUrl, int port) {
        ...
        String protocol = baseUrl.substring(0, pos + 2);
        String domain = baseUrl.substring(pos + 2);
        ...
        if (portPos >= 0) {
            domain = domain.substring(0, portPos);
        }
    ...
    private byte[] computeSignature(String baseString)
        ...
        // Create key.
        String keyString = RestUtils.percentEncode(consumerSecret) + '&' + RestUtils.percentEncode("");
