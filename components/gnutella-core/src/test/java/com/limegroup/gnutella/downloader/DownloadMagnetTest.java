...
public class DownloadMagnetTest extends DownloadTestCase {
    ...
    private void publishPushProxyForGuid(MojitoDHT dht, GUID guid, int proxyPort, int clientPort) throws Exception {
        PushProxiesValue value = new PushProxiesValueImpl(dht.getVersion(), guid.bytes(), (byte) 0, UDPConnection.VERSION, clientPort, Collections.singleton(new IpPortImpl("127.0.0.1", proxyPort)));
