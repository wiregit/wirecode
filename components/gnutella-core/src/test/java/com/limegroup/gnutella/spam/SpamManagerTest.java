...
public class SpamManagerTest extends LimeTestCase {
    ...
    private TestResponseVerifier testResponseVerifier;
    ...
    protected void setUp() throws Exception {
        ...
        testResponseVerifier = injector.getInstance(TestResponseVerifier.class);
    ...
    public void testClientGUIDAffects() throws Exception {
        ...
        RemoteFileDesc marked =
            createRFD(addr1, port1, badger, null, urn1, size1, guid1, null);
        manager.handleUserMarkedSpam(new RemoteFileDesc[]{marked});
        assertTrue(marked.isSpam());
        assertGreaterThan(0f, marked.getSpamRating());
        ...
        RemoteFileDesc sameGUID =
            createRFD(addr2, port2, mushroom, null, urn2, size2, guid1, null);
        ...
        RemoteFileDesc unrelated =
            createRFD(addr3, port3, snake, null, urn3, size3, guid2, null);
        assertFalse(unrelated.isSpam());
        assertEquals(0f, unrelated.getSpamRating());
    }

    /**
     * Tests that the template of the result affects the spam rating
     */
    public void testTemplateAffects() throws Exception {
        String badgerFromTemplate = "123 " + badger + " 456.xyz";
        String mushroomFromTemplate = "123 " + mushroom + " 456.xyz";

        // The user searches for badgers
        testResponseVerifier.guid = GUID.makeGuid();
        testResponseVerifier.query = badger;

        // A result arrives and the user marks it as spam
        RemoteFileDesc marked =
            createRFD(addr1, port1, badgerFromTemplate, null, urn1, size1,
                    GUID.makeGuid(), testResponseVerifier.guid);
        manager.handleUserMarkedSpam(new RemoteFileDesc[]{marked});
        assertTrue(marked.isSpam());
        assertGreaterThan(0f, marked.getSpamRating());

        // The user searches for mushrooms
        testResponseVerifier.guid = GUID.makeGuid();
        testResponseVerifier.query = mushroom;

        // A result with nothing in common with the first result except the
        // template should receive a non-zero rating
        RemoteFileDesc sameGUID =
            createRFD(addr2, port2, mushroomFromTemplate, null, urn2, size2,
                    GUID.makeGuid(), testResponseVerifier.guid);
        assertGreaterThan(0f, sameGUID.getSpamRating());
        assertLessThan(marked.getSpamRating(), sameGUID.getSpamRating());
        ...
        // A result with nothing in common should receive a zero spam rating
        RemoteFileDesc unrelated =
            createRFD(addr3, port3, snake, null, urn3, size3,
                    GUID.makeGuid(), testResponseVerifier.guid);
        assertFalse(unrelated.isSpam());
        assertEquals(0f, unrelated.getSpamRating());
    }
    ...
    throws Exception {
        ...
        RemoteFileDesc rfd =
            remoteFileDescFactory.createRemoteFileDesc(
                    ...
                    clientGUID, 3, 3, false, doc, urns, false, "ALT", 0,
                    true, queryGUID);
