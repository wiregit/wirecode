...
public class PingReplyTest extends TestCase {
     ...
     *  these can be decoded.  Note that this will need to be changed if
     *  more extensions are added. */
    public void testGGEPEncodeDecode2() {
        //Create pong
        PingReply pr=new PingReply(new byte[16], (byte)3, 6349, new byte[4],
        ...
        //...etc.
        //little byte of 523
        assertTrue(bytes[bytes.length-2-(5+upLength)-(7+vcLength)]==(byte)0x0B);
        //big byte of 523
        assertTrue(bytes[bytes.length-1-(5+upLength)-(7+vcLength)]==(byte)0x02);
