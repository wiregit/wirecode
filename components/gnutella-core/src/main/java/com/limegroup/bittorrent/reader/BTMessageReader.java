...
/**
 * Implements a ChannelReadObserver for the a given BTConnection. All data read
 * from the BTConnection is processed by this read observer. It handles parsing
 * the data and updating the state of the connection, and forwarding the changes
...
public class BTMessageReader implements ChannelReadObserver, PieceParseListener {
    ...
    /**
     * Whether we've stopped reading from the network (not same as connection
     * choking)
     */
    ...
    /**
     * <tt>ThreadPool</tt> on which its safe to schedule calls to handleRead
     */
    ...
    public BTMessageReader(IOErrorObserver ioxObserver, BTMessageHandler handler,
            ScheduledExecutorService networkInvoker, ByteBufferCache cache) {
    ...
    /**
     * chokes or unchokes read signals from this channel. Not the same as
     * torrent connection choking
     * 
    ...
    /**
     * Runnable that explicitly calls handleRead to process any data that was
     * read after the end of a piece.
     */
