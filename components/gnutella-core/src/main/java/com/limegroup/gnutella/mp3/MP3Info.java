...
public final class MP3Info {
	...
	/**
	 * memory buffer reuse; reuse 4096 byte buffer to read multiple MP3 files
	 * @experimental
	 * @see #checkoutReusableBytes
	 * @see #checkinReusableBytes
	 */
	private static SoftReference RU_BYTES;
    ...
    public MP3Info(String file) 
		...
		byte[] buf = checkoutReusableBytes(); //new byte[2048];
	...
	//  Looking for the VBR
        ...
        switch (buf[i+0]) {
			...
			try {
				checkinReusableBytes(buf);
				fis.close(); 
			} catch (Exception e) {}//ignore
		}
    }
	/**
     * Attempt to reuse a previous buffer array
     *
     * @param byte[]
     */
    private void checkinReusableBytes(byte[] b) {

	    if (RU_BYTES == null || RU_BYTES.get() == null) {
		     RU_BYTES = new SoftReference(b);
	    }
	}
	/**
     * Attempt to reuse a previous buffer array
     *  new array created if the previous can't be fetched or gc'ed
     *
     * @return byte[]
     */
    private synchronized byte[] checkoutReusableBytes() {

	    if (RU_BYTES == null || RU_BYTES.get() == null) {
		    return  new byte[4096];
	    }
	    else {
		    return (byte[]) RU_BYTES.get();
	    }
	}
    public int getBitRate() {
  ...
  //???? If computed bitrate is nonsensical, just use header bitrate 
	 ...
	 * ::Example::  LAME Tag
	 *
	 *   0005 LAME3.90. õYa kY1 õYq 7Y3 dY2 pYi 0
