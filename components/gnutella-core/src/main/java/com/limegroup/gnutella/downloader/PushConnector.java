...
class PushConnector extends HTTPConnectObserver {
     ...
     * @param worker TODO
     */
    PushConnector(DownloadWorker worker, boolean forgetOnFailure, boolean directConnectOnFailure) {
        this.worker = worker;
    ...
    public void handleConnect(Socket socket) {
        ...
        HTTPDownloader dl = new HTTPDownloader(socket, this.worker._rfd, this.worker._commonOutFile, this.worker._manager instanceof InNetworkDownloader);
        try {
           dl.connectTCP(0);
     ...
     */
    private void failed() {            
        this.worker._manager.unregisterPushObserver(pushDetails, false);
        
        if(!directConnectOnFailure) {
            if(forgetOnFailure) {
                this.worker._manager.forgetRFD(this.worker._rfd);
            }
