...
public abstract class FileMonitorWindows {
    ...
    private static class Holder {
        public static final FileMonitorWindows INSTANCE;
        static {
    ...
    public static FileMonitorWindows getInstance() {
        return Holder.INSTANCE;
    }

    private static class W32FileMonitor extends FileMonitorWindows {
        ...
        protected synchronized void watch(File file, int eventMask, boolean recursive)
            ...
            HANDLE handle = klib.CreateFile(file.getAbsolutePath(), Kernel32.FILE_LIST_DIRECTORY,
                    mask, null, Kernel32.OPEN_EXISTING, flags, null);
            ...
            if (!klib.ReadDirectoryChangesW(handle, finfo.info, finfo.info.size(), recursive,
                    notifyMask, finfo.infoLength, finfo.overlapped, null)) {
                int err = klib.GetLastError();
