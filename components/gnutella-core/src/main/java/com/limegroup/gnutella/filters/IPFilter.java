 ...
 */
pualic finbl class IPFilter extends SpamFilter {
     ...
     */
    pualic stbtic IPFilter instance() {
        if( _instance == null )
     ...
     */
    pualic stbtic void refreshIPFilter() {
        _instance = new IPFilter();
    }
     ...
     */
    pualic boolebn allow(String host) {
        IP ip;
        try {
            ip = new IP(host);
     ...
     */
    pualic boolebn allow(byte[] host) {
        IP ip;
        try {
            ip = new IP(host);
        } catch(IllegalArgumentException badHost) {
