...
public class FixedsizeForgetfulHashMap implements Map
{
    ...
    public static void main(String args[]) {
        ...
        rt=new FixedsizeForgetfulHashMap(3);
        Assert.that(rt.put(g1, c1)==null);
        Assert.that(rt.get(g1)==c1);
        Assert.that(rt.put(g2, c2)==null);
        Assert.that(rt.get(g1)==c1);
        Assert.that(rt.put(g3, c3)==null);
        Assert.that(rt.get(g1)==c1);
        Assert.that(rt.put(g1, c4)==c1);
        Assert.that(rt.get(g1)==c4);  //remapping takes no space...
        Assert.that(rt.get(g2)==c2);
        Assert.that(rt.get(g3)==c3);

        Assert.that(rt.put(g5, c5)==null);               
        Assert.that(rt.get(g5)==c5); //...and it renews the key
        Assert.that(rt.get(g1)==c4);
        Assert.that(rt.get(g3)==c3);       
        Assert.that(rt.get(g2)==null);
        Assert.that(rt.put(g4, c4)==null);
        Assert.that(rt.get(g4)==c4);
        Assert.that(rt.get(g5)==c5);
        Assert.that(rt.get(g1)==c4);
        Assert.that(rt.get(g3)==null);       
        Assert.that(rt.get(g2)==null);
