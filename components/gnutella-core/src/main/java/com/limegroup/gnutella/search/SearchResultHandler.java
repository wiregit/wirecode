...
public final class SearchResultHandler {
    ...
    public void removeQuery(GUID guid) {
        ...
        if ((gc != null) && (!gc.isFinished())) {
            ...
            QueryStatusResponse stat = new QueryStatusResponse(guid, MAX_RESULTS);
    ...
    private boolean handleReply(final QueryReply qr) {
        ...
        if( !data.isReplyToMulticastQuery() && !qr.isBrowseHostReply() ) {
            ...
            if (data.isFirewalled() && 
                ...
                (!RouterService.acceptedIncomingConnection() || NetworkUtils.isPrivateAddress(RouterService.getAddress())) &&
    ...
    private void accountAndUpdateDynamicQueriers(final QueryReply qr, final int numSentToFrontEnd) {
        ...
        if (numSentToFrontEnd > 0) {
            ...
            if (RouterService.isShieldedLeaf()) {
                ...
                if (!gc.isFinished() && (gc.getNumResults() > gc.getNextReportNum())) {
                    ...
                    final int numResultsToReport = (gc.isFinished() ? MAX_RESULTS : gc.getNumResults() / 4);
    ...
    private boolean isQueryStillValid(GuidCount gc, long now) {
        ...
        return (now < (gc.getTime() + QUERY_EXPIRE_TIME)) && (gc.getNumResults() < QueryHandler.ULTRAPEER_RESULTS);
    }
    ...
    private static class GuidCount {
        ...
        public void tallyReport() { _nextReportNum = _numResults + REPORT_INTERVAL; }
