 ...
 * uptime.  These are stored in the gnutella.net file.  
 */
public class HostCatcher {    
    ...
    /** The list of pongs with the highest uptimes.  Each hosts weight is set to
     * the uptime.  These are most likely to be reachable during the next
     * session, though not necessarily likely to have slots available now.  In
     * this way, they act more like bootstrap hosts than normal pongs.  This
     * list is written to gnutella.net and used to initialize queue on startup.
     *
    ...
    synchronized void read(String filename)
        ...
        for (int i=0; i<SIZE; i++) {
            ...
            //Break the line into host, port, and optional uptime.  Skip if
            //badly formatted.
    ...
    /** Returns the expected uptime of the given node. */
    private static int expectedUptime(Endpoint e, boolean ultrapeer) {
        //By looking at version logs, we find that the average uptime for a host
        //is about 8.1 minutes.  A study of connection uptimes
