...
public class SearchResultHandlerImpl implements SearchResultHandler {
    ...
    final Provider<SpamManager> spamManager;
    ...
    private static class GuidCount implements SearchResultStats {
        ...
        public int addQueryReply(SearchResultHandler srh, QueryReply qr, HostData data) {
            ...
            for (Response response : results) {
                ...
                if (skipSpam || !srh.getSpamManager().get().isSpam(rfd)) {
                    if (is != null)
                        addIntervalSet(response.getUrns(), is);
                    else
                        numGood++;
                }
                else
            ...
            // TODO
            //
            // replace this with a call to a function that determines
            // how many times a file is fully represented be the set
            // of known partial results.
            //
            if (_isets.size() != 0)
                _numGoodResults++;
         ...
         * @return Returns the URN that was used as the key for the IntervalSet
         * 
         */
        private URN addIntervalSet (Set<URN> urns, IntervalSet is) {
            URN ret = null;
