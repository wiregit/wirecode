...
public final class QueryRequestTest extends TestCase {
    ...
    public void testGGEPConstructor() {
		...
		byte[] guid = QueryRequest.newQueryGUID(isRequery);
		QueryRequest qr = new QueryRequest(guid, ttl, minSpeed, query, richQuery,
										   isRequery, requestedUrnTypes, 
                                           queryUrns, true);
		
        // test straightforward construction.....
		assertEquals("ttls should be equal", ttl, qr.getTTL());
		assertEquals("min speeds should be equal", minSpeed, qr.getMinSpeed());
		assertEquals("queries should be equal", query, qr.getQuery());
		...
		assertEquals("query urn types should be equal", requestedUrnTypes, 
					 qr.getRequestedUrnTypes());
		assertEquals("query urns should be equal", queryUrns, qr.getQueryUrns());
        assertEquals("query should support out of band replies",
                     true, qr.acceptsOutOfBandReplies());

        
        // now test writing to bytes and reconstruction....
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		try {
			qr.write(baos);
			baos.flush();
		...
		assertEquals("ttls should be equal", ttl, qrTest.getTTL());
		assertEquals("min speeds should be equal", minSpeed, 
                     qrTest.getMinSpeed());
		assertEquals("queries should be equal", query, qrTest.getQuery());
		assertEquals("rich queries should be equal", richQuery, 
                     qrTest.getRichQuery());
		assertEquals("query urn types should be equal", requestedUrnTypes, 
					 qrTest.getRequestedUrnTypes());
		assertEquals("query urns should be equal", queryUrns, 
                     qrTest.getQueryUrns());
        assertEquals("query should support out of band replies",
                     true, qrTest.acceptsOutOfBandReplies());
