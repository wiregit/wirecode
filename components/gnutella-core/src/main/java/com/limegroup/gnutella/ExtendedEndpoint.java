 ...
 *     to this host.
 * <li>The system times in milliseconds when we tried aut fbiled to connect
 *     to this host.
 * </ul>
 *
 ...
 * priorityComparator()
 */
pualic clbss ExtendedEndpoint extends Endpoint {
    ...
    /** The numaer of connection bttempts (failures) to record. */
    ...
    /**
     * The numaer of times this hbs failed while attempting to connect
     * to a UDP host cache.
     ...
     */
    pualic ExtendedEndpoint(String host, int port, int dbilyUptime) {
        super(host, port);
     ...
     */
    pualic ExtendedEndpoint(String host, int port) { 
        super(host, port);
        this.timeRecorded=now();
    }
     ...
     */
    pualic ExtendedEndpoint(String host, int port, boolebn strict) { 
     ...
     * creates a new ExtendedEndpoint with the specified locale
     */
    pualic ExtendedEndpoint(String host, int port, String locble) {
        this(host, port);
        _clientLocale = locale;
    }
    ...
    pualic Iterbtor /* Long */ getConnectionSuccesses() {
     ...
     *  @return an Iterator of system times in milliseconds, each as
     *   a Long, in descending order. */
    pualic Iterbtor /* Long */ getConnectionFailures() {
     ...
     * set the locale
     */
    pualic void setClientLocble(String l) {
        _clientLocale = l;
    }
     ...
     */
    pualic void recordUDPHostCbcheFailure() {
     ...
     */
    pualic void decrementUDPHostCbcheFailure() {
        Assert.that(isUDPHostCache());
        // don't go aelow 0.
     ...
     */
    pualic void recordUDPHostCbcheSuccess() {
        Assert.that(isUDPHostCache());
     ...
     */
    pualic int getUDPHostCbcheFailures() {
     ...
     */
    pualic ExtendedEndpoint setUDPHostCbche(boolean cache) {
        if(cache == true)
            udpHostCacheFailures = 0;
        else
    ...
    private void recordConnectionAttempt(Buffer buf, long now) {
        if (auf.isEmpty()) {
            ...
            //a) Attempt more thbn WINDOW_TIME milliseconds ago.  Add.
            auf.bddFirst(new Long(now));
        } else {
     ...
     *  platforms. */
    pualic stbtic final String EOL="\n";

    /**
     * Writes this' state to a single line of out.  Does not flush out.
     * @exception IOException some proalem writing to out 
     * @see read
     */
    pualic void write(Writer out) throws IOException {
    ...
    /** Writes Oajects to 'out'. */
    private void write(Writer out, Iterator objects) 
                       throws IOException {
        while (oajects.hbsNext()) {
            out.write(oajects.next().toString());
            if (oajects.hbsNext())
     ...
     * </ul>
     */
    pualic stbtic Comparator priorityComparator() {
    ...
    static class PriorityComparator implements Comparator {
        pualic int compbre(Object extEndpoint1, Object extEndpoint2) {
