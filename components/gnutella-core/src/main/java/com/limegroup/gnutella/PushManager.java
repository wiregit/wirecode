...
public final class PushManager {
    ...
    /**
     * Accepts a new push upload. NON-BLOCKING: creates a new thread to transfer
     * the file.
     * <p>
     * The thread connects to the other side, waits for a GET/HEAD, and
     * delegates to the UploaderManager.acceptUpload method with the socket it
     * created. Essentially, this is a reverse-Acceptor.
     * <p>
     ...
     * @param lan whether or not this is a request over a local network ( (force
     *        the UploadManager to accept this request when it comes back)
     * @param isFWTransfer whether or not to use a UDP pipe to service this
     *        upload.
     */
    public void acceptPushUpload(final String host, final int port,
            final String guid, final boolean lan, final boolean isFWTransfer) {
        if (LOG.isDebugEnabled())
            LOG.debug("Accepting Push Upload from ip:" + host + " port:" + port
                    + " FW:" + isFWTransfer);
        ...
        // If the transfer is to be done using FW-FW, then immediately start a
        // new thread
        // which will connect using FWT. Otherwise, do a non-blocking connect
        // and have
        // the observer spawn the thread only if it succesfully connected.
        if (isFWTransfer) {
            if (LOG.isDebugEnabled())
                LOG.debug("Adding push observer FW-FW to host: " + host + ":"
                        + port);
            UDPConnection socket = new UDPConnection();
            socket.connect(new InetSocketAddress(host, port),
                    CONNECT_TIMEOUT * 2, new PushObserver(data, true));
        } else {
            if (LOG.isDebugEnabled())
                LOG.debug("Adding push observer to host: " + host + ":" + port);
            try {
                Sockets.connect(host, port, CONNECT_TIMEOUT, new PushObserver(
                        data, false));
    ...
    private static class PushObserver implements ConnectObserver {
        ...
        public void shutdown() {
            if (LOG.isDebugEnabled())
                LOG.debug("Push (fwt: " + fwt + ") connect to: "
                        + data.getHost() + ":" + data.getPort() + " failed");
            if (fwt)
        ...
        public void handleConnect(Socket socket) throws IOException {
            if (LOG.isDebugEnabled())
                LOG.debug("Push (fwt: " + fwt + ") connect to: "
                        + data.getHost() + ":" + data.getPort() + " succeeded");
    ...
    private static class PushConnector implements ChannelWriter {
        ...
        public PushConnector(Socket socket, PushData data) {
            this.data = data;
            this.socket = socket;
        ...
        public boolean handleWrite() throws IOException {
            ...
            if (buffer.hasRemaining()) {
                ...
                finished = true;
                RouterService.getUploadManager().acceptUpload(null, socket,
                        data.isLan());
                return false;
            }
        }
            ...
            try {
                ...
                if (word.equals("GET")) {
                    ...
                    RouterService.getUploadManager().acceptUpload(
                            HTTPRequestMethod.GET, socket, data.isLan());
                } else if (word.equals("HEAD")) {
                    UploadStat.PUSHED_HEAD.incrementStat();
                    RouterService.getUploadManager().acceptUpload(
                            HTTPRequestMethod.HEAD, socket, data.isLan());
                } else {
                ...
                if (LOG.isDebugEnabled())
                    LOG.debug("Failed push connect/transfer to "
                            + data.getHost() + ":" + data.getPort() + ", fwt: "
                            + fwTransfer);
                if (fwTransfer)
                    UploadStat.FW_FW_FAILURE.incrementStat();
                UploadStat.PUSH_FAILED.incrementStat();
