...
public class PatriciaTrie extends AbstractMap implements SortedMap, Serializable {
    ...
    private final TrieEntry root = new TrieEntry(null, null, -1);
    ...
    public Object put(Object key, Object value) {
        ...
        TrieEntry found = getNearestEntryForKey(key, keyLength);
    ...
    private TrieEntry addEntry(TrieEntry toAdd, int keyLength) {
     ...
     */
    private TrieEntry getNearestEntryForKey(Object key, int keyLength) {
        TrieEntry current = root.left;
        TrieEntry path = root;
        while(true) {
            if(current.bitIndex <= path.bitIndex)
    ...
    private boolean selectR(TrieEntry h, int bitIndex, 
            final Object key, final int keyLength, final TrieEntry[] result) {
    ...
    public Map.Entry select(Object key, Cursor cursor) {
        int keyLength = length(key);
        ...
        if (h.bitIndex <= bitIndex) {
            if(!h.isEmpty()) {
                ...
                case Cursor.EXIT:
                    result[0] = h;
                    return false; // exit
                case Cursor.REMOVE_AND_EXIT:
                    TrieEntry entry = new TrieEntry(h.getKey(), h.getValue(), -1);
                    result[0] = entry;
                    removeEntry(h);
                    return false;
     ...
     * @return
     */
    public SortedMap getPrefixedBy(Object key) {
     ...
     * @return
     */
    public SortedMap getPrefixedBy(Object key, int length) {
     ...
     * @return
     */
    public SortedMap getPrefixedBy(Object key, int offset, int length) {
     ...
     * @return
     */
    public SortedMap getPrefixedByBits(Object key, int bitLength) {
     ...
     * @return
     */
    private SortedMap getPrefixedByBits(Object key, int offset, int length) {
     ...
     * to remove)
     */
    private Object removeEntry(TrieEntry h) {
        if (h != root) {
            if (h.isInternalNode()) {
    ...
    private void removeExternalEntry(TrieEntry h) {
        ...
        TrieEntry child = (h.left == h) ? h.right : h.left;
    ...
    private void removeInternalEntry(TrieEntry h) {
        ...
        // Fix P's parent, predecessor and child Nodes
        {
            ...
            TrieEntry child = (p.left == h) ? p.right : p.left;
     ...
     */
    private TrieEntry nextEntry(TrieEntry node) {
        if(node == null) {
            return firstEntry();
        } else {
     ...
     */
    private TrieEntry nextEntryImpl(TrieEntry start, TrieEntry previous, TrieEntry tree) {
    ...
    public Map.Entry traverse(Cursor cursor) {
        TrieEntry entry = nextEntry(null);
        while(entry != null) {
            TrieEntry current = entry;
            int ret = cursor.select(current);
            ...
            case Cursor.REMOVE_AND_EXIT:
                Map.Entry value = new TrieEntry(current.getKey(), current.getValue(), -1);
                removeEntry(current);
                return value;
            case Cursor.CONTINUE: // do nothing.
            }
        }
    ...
    private boolean isValidUplink(TrieEntry next, TrieEntry from) {            
     ...
     */
    private boolean isBitSet(Object key, int keyLength, int bitIndex) {
    ...
    private static class TrieEntry implements Map.Entry, Serializable {
        ...
        private TrieEntry(Object key, Object value, int bitIndex) {
            this.key = key;
            this.value = value;
    ...
    public static interface KeyAnalyzer extends Comparator, Serializable {
         ...
         */
        public int bitIndex(Object key, int keyStart, int keyLength,
                            Object found, int foundStart, int foundLength);
    ...
    private abstract class NodeIterator implements Iterator {
        ...
        protected TrieEntry next; // the next node to return
        protected TrieEntry current; // the current entry we're on
        ...
        protected NodeIterator(TrieEntry firstEntry) {
            next = firstEntry;
        }
    ...
    private class KeyIterator extends NodeIterator {
        public Object next() {
            return nextEntry().getKey();
        }
    }

    private class EntryIterator extends NodeIterator {
        public Object next() {
            return nextEntry();
        }
    }
    ...
    private class PrefixEntryIterator extends NodeIterator {
        ...
        PrefixEntryIterator(TrieEntry startScan, Object prefix, int offset, int length) {
            subtree = startScan;
    ...
    private class SubMapEntryIterator extends NodeIterator {
        private final Object firstExcludedKey;

        SubMapEntryIterator(TrieEntry first, TrieEntry firstExcluded) {
            super(first);
            firstExcludedKey = 
    ...
    private class EntrySet extends AbstractSet {
        ...
        public boolean contains(Object o) {
            ...
            TrieEntry candidate = getEntry(((Map.Entry)o).getKey());
    ...
    private TrieEntry followLeft(TrieEntry node) {
        while(true) {
            TrieEntry child = node.left;
    ...
    protected TrieEntry followRight(TrieEntry node) {
     ...
     */
    protected TrieEntry higherEntry(Object key) {
     ...
     */
    protected TrieEntry ceilingEntry(Object key) {
        // Basically:
     ...
     */
    protected TrieEntry lowerEntry(Object key) {
        // Basically:
        // Follow the steps of adding an entry, but instead...
        //
        // - If we ever encounter a situation where we found an equal
     ...
     */
    protected TrieEntry floorEntry(Object key) {        
        // TODO: Cleanup so that we don't actually have to add/remove from the
        //       tree.  (We do it here because there are other well-defined 
        //       functions to perform the search.)
        int keyLength = length(key);
        ...
        if (isValidBitIndex(bitIndex)) {
            ...
            addEntry(added, keyLength);
            incrementSize(); // must increment because remove will decrement
            TrieEntry floor = previousEntry(added);
            removeEntry(added);
            modCount -= 2; // we didn't really modify it.
            return floor;
        } else if (isNullBitKey(bitIndex)) {
     ...
     */
    private TrieEntry subtree(Object prefix, int offset, int length) {
        TrieEntry current = root.left;
        TrieEntry path = root;
        while(true) {
    ...
    private class PrefixSubMap extends SubMap {
        ...
        PrefixSubMap(Object prefix, int offset, int length) {
            this.prefix = prefix;
            this.offset = offset;
            this.length = length;
            fromInclusive = false;
    ...
    private class SubMap extends AbstractMap implements SortedMap, java.io.Serializable {
        ...
        public boolean containsKey(Object key) {
            return inRange(key) && PatriciaTrie.this.containsKey(key);
        }
