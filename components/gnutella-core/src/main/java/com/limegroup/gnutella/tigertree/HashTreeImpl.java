...
class HashTreeImpl implements Serializable, HashTree {
     ...
     */
    public boolean isCorrupt(Range in, FileStream fileStream, ByteBuffer buffer) {
        ...
        if (in.getLow() % _nodeSize == 0 && 
            ...
            try {
                MerkleTree digest = new MerkleTree(new Tiger());
                long startPosition = in.getLow();
                while(startPosition <= in.getHigh()) {
                    buffer.clear();
                    int size = (int)Math.min(buffer.remaining(), in.getHigh() - startPosition + 1);
                    buffer.limit(size);
                    fileStream.read(buffer, startPosition);
                    buffer.flip();
