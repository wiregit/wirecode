 ...
 */
public class GnutellaConnection extends AbstractConnection 
	implements ReplyHandler, MessageReceiver, SentMessageHandler, Shutdownable,
    Inspectable, RoutedConnection, ConnectionRoutingStatistics, ConnectionMessageStatistics {
    ...
    public GnutellaConnection(String host, int port, ConnectType type,
        ...
        super(host, port, type, capabilitiesVMFactory, supportedVendorMessage, networkManager, acceptor);
    ...
    public void initialize(Properties requestHeaders, HandshakeResponder responder, int timeout,
        ...
        if (isOutgoing()) {
            ConnectObserver connectObserver = new AsyncHandshakeConnecter(requestHeaders, responder, observer);
            InetSocketAddress host = new InetSocketAddress(getAddress(), getPort());
            Socket socket = socketsManager.connect(host, timeout, connectObserver, getConnectType());
            setSocket(socket);
        } else {
     ...
     */
    private void startHandshake(Properties requestHeaders, HandshakeResponder responder, GnetConnectObserver observer)
      throws IOException {
        initializeHandshake();
        ...
        Handshaker shaker;        
        if(isOutgoing())
            shaker = new AsyncOutgoingHandshaker(requestHeaders, responder, getSocket(), shakeObserver);
        else
    ...
    private void postHandshakeInitialize(Handshaker shaker) {
        ...
        getConnectionBandwidthStatistics().setCompressionOption(isWriteDeflated(), isReadDeflated(), new CompressionBandwidthTrackerImpl(inflater, deflater));
    ...
    public boolean shouldForwardQuery(QueryRequest query) {
        ...
        if (query.isFeatureQuery()) {
            if (isSupernodeClientConnection())
                return (getConnectionCapabilities().getRemoteHostFeatureQuerySelector() >= 
    ...
    private QueryRequest tryToProxy(QueryRequest query) {
        ...
        if (getConnectionCapabilities().remoteHostSupportsLeafGuidance() < 1) return query;
    ...
    /* (non-Javadoc)
     * @see com.limegroup.gnutella.RoutedConnection#handlePushRequest(com.limegroup.gnutella.messages.PushRequest, com.limegroup.gnutella.ReplyHandler)
     */
    ...
    public void measureBandwidth() {
        _upBandwidthTracker.measureBandwidth(
             ByteOrder.long2int(getConnectionBandwidthStatistics().getBytesSent()));
        _downBandwidthTracker.measureBandwidth(
             ByteOrder.long2int(getConnectionBandwidthStatistics().getBytesReceived()));
    }
    ...
    // all ReplyHandler things, to override, override from ConnectionCapabilities...
