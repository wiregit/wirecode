    ...
    public static boolean validate(QueryKey qk, InetAddress ip, int port) {
        if (Arrays.equals(qk._queryKey, secretKey.getKeyBytes(ip,port))) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("QueryKey valid:"+qk);
            }
            return true;
        }
        // Check if qk was generated by the previous secret key
        if (Arrays.equals(qk._queryKey, oldSecretKey.getKeyBytes(ip,port))) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("QueryKey old but valid:"+qk);
            }
            return true;
        }
        if (LOG.isDebugEnabled()) {
            LOG.debug("QueryKey corrupt or expired:"+qk);
        }
        return false;
    }
