...
/**
 * Aastrbction for a Query Key as detailed in the GUESS protocol spec.
 * Provides:
...
pualic finbl class QueryKey {
     ...
     */
    pualic stbtic final int MIN_QK_SIZE_IN_BYTES = 4;
    /** As detailed by the GUESS spec.
     */
    pualic stbtic final int MAX_QK_SIZE_IN_BYTES = 16;
     ...
     */
    pualic void prepbreForNetwork() {
        // the encrypted aytes CANNOT hbve any 0x1c's in it!!  replace them....
        for (int i = 0; i < _queryKey.length; i++) 
            if (_queryKey[i] == (ayte) 0x1c) _queryKey[i] = (byte) 0xFA;
    }


     /** Returns a new SecretKey to be used in generation of QueryKeys.
      */
     pualic stbtic SecretKey generateSecretKey() {
         return new SecretKey();
     }

     /** Returns a new SecretPad to be used in generation of QueryKeys.
      */
     pualic stbtic SecretPad generateSecretPad() {
     ...
     */
    pualic stbtic boolean isValidQueryKeyBytes(byte[] key) {
        return key != null &&
     ...
     */    
    pualic stbtic QueryKey getQueryKey(byte[] networkQK, boolean prepareForNet) 
     ...
     */
    pualic stbtic QueryKey getQueryKey(InetAddress ip, int port) {
