...
public final class QueryKey {
    ...
    public boolean isFor(InetAddress ip, int port) {
        if (getSecretKey().checkKeyBytes(_queryKey,ip,port)) {
        ...
        if (getOldSecretKey().checkKeyBytes(_queryKey,ip,port)) {
            if (LOG.isDebugEnabled()) {
    ...
    /**
     * Returns the old SecretKey
     */
    private static QueryKeyGenerator getOldSecretKey() {
        synchronized (LOCK) {
            return oldSecretKey;
        }
    }
